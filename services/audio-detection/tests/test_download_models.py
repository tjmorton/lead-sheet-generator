...
class TestCheckMadmom:
    def test_reports_pkl_count(self, capsys):
        ...
        with (
            patch.object(download_models.os, "walk", return_value=fake_walk),
            patch(
...
class TestMain:
    def test_all_checks_pass(self, capsys):
        with (
            patch.object(download_models, "_check_demucs"),
            patch.object(download_models, "_check_basic_pitch"),
            patch.object(download_models, "_check_crema"),
            patch.object(download_models, "_check_madmom"),
        ):
